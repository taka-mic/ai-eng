...
export function HomeScreen({ navigation }: Props) {
  ...
  const accuracy = progress.totalAnswered > 0 ? Math.round((progress.correctAnswered / progress.totalAnswered) * 100) : 0;

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <Card elevated style={styles.statusCard}>
        <View style={styles.levelRow}>
          <View style={styles.levelBadge}><Text style={styles.levelNumber}>{progress.level}</Text></View>
          <View style={styles.levelInfo}>
            <Text style={styles.levelLabel}>レベル {progress.level}</Text>
            <Text style={styles.xpText}>{xpInCurrentLevel} / 200 XP</Text>
          </View>
      ...
      <View style={styles.statsRow}>
        <StatCard icon="checkmark-circle" iconColor={Colors.success} value={progress.totalAnswered} label="回答数" />
        <StatCard icon="trophy" iconColor={Colors.warning} value={`${accuracy}%`} label="正答率" />
        <StatCard icon="flag" iconColor={Colors.primary} value={progress.completedQuestIds.length} label="クリア" />
      </View>
      <Text style={styles.sectionTitle}>おすすめクエスト</Text>
      {allQuests.slice(0, 4).map((quest) => (
        <QuestListItem key={quest.id} quest={quest} completed={progress.completedQuestIds.includes(quest.id)} onPress={() => navigation.navigate('Quest', { questId: quest.id })} />
      ))}
    </ScrollView>
  );
}

function StatCard({ icon, iconColor, value, label }: { icon: React.ComponentProps<typeof Ionicons>['name']; iconColor: string; value: number | string; label: string }) {
  return (<Card style={styles.statCard}><Ionicons name={icon} size={24} color={iconColor} /><Text style={styles.statValue}>{value}</Text><Text style={styles.statLabel}>{label}</Text></Card>);
}

function QuestListItem({ quest, completed, onPress }: { quest: Quest; completed: boolean; onPress: () => void }) {
    ...
    <TouchableOpacity onPress={onPress} activeOpacity={0.85}>
      <Card style={styles.questCard}>
        <View style={styles.questLeft}>
          <View style={styles.questTypeIcon}><Ionicons name={quest.type === 'vocabulary' ? 'book' : 'pencil'} size={20} color={Colors.primary} /></View>
          <View style={styles.questInfo}>
            <Text style={styles.questTitle}>{quest.title}</Text>
            <Text style={styles.questDesc} numberOfLines={1}>{quest.description}</Text>
            <View style={styles.questMeta}>
              <View style={[styles.diffBadge, { backgroundColor: diffColor + '22' }]}><Text style={[styles.diffText, { color: diffColor }]}>{diffLabel}</Text></View>
              <Text style={styles.questMetaText}>{quest.totalQuestions}問 · {quest.xpReward} XP</Text>
            </View>
          </View>
        </View>
        {completed ? <Ionicons name="checkmark-circle" size={24} color={Colors.success} /> : <Ionicons name="chevron-forward" size={20} color={Colors.textMuted} />}
      </Card>
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  ...
  levelBadge: { width: 48, height: 48, borderRadius: Theme.borderRadius.full, backgroundColor: Colors.primary, alignItems: 'center', justifyContent: 'center', marginRight: Theme.spacing.md },
  ...
  streakBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#FFF5E4', paddingHorizontal: Theme.spacing.sm, paddingVertical: 4, borderRadius: Theme.borderRadius.full },
  ...
  questTypeIcon: { width: 44, height: 44, borderRadius: Theme.borderRadius.md, backgroundColor: Colors.primaryLight, alignItems: 'center', justifyContent: 'center' },
  questInfo: { flex: 1 },
