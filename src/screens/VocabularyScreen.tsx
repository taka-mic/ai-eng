...
export function VocabularyScreen({ navigation }: Props) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <Text style={styles.subtitle}>入試に出る必須英単語を体系的に学習しよう。クエストをクリアしてレベルアップ！</Text>
      {vocabQuests.map((quest) => (
        <QuestCard key={quest.id} quest={quest} completed={progress.completedQuestIds.includes(quest.id)} onPress={() => navigation.navigate('Quest', { questId: quest.id })} />
      ))}
    </ScrollView>
  );
}

function QuestCard({ quest, completed, onPress }: { quest: Quest; completed: boolean; onPress: () => void }) {
    ...
    <TouchableOpacity onPress={onPress} activeOpacity={0.85}>
      <Card elevated style={styles.card}>
        <View style={styles.header}>
          ...
          {completed && <View style={styles.completedBadge}><Ionicons name="checkmark-circle" size={16} color={Colors.success} /><Text style={styles.completedText}>クリア済み</Text></View>}
        </View>
        <Text style={styles.title}>{quest.title}</Text>
        <Text style={styles.desc}>{quest.description}</Text>
        <View style={styles.footer}>
          <View style={styles.footerItem}><Ionicons name="help-circle-outline" size={14} color={Colors.textMuted} /><Text style={styles.footerText}>{quest.totalQuestions}問</Text></View>
          <View style={styles.footerItem}><Ionicons name="star-outline" size={14} color={Colors.warning} /><Text style={styles.footerText}>{quest.xpReward} XP</Text></View>
          <View style={styles.startButton}><Text style={styles.startText}>{completed ? '再挑戦' : 'スタート'}</Text><Ionicons name="arrow-forward" size={14} color={Colors.textOnPrimary} /></View>
        </View>
      </Card>
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  ...
  startButton: { marginLeft: 'auto', flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: Colors.primary, paddingHorizontal: Theme.spacing.md, paddingVertical: 6, borderRadius: Theme.borderRadius.full },
