  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <Card elevated style={styles.levelCard}>
        <View style={styles.levelTop}>
          ...
          <View style={styles.levelDetails}>
            ...
            <Text style={styles.nextLevel}>次のレベルまで {200 - xpInCurrentLevel} XP</Text>
          </View>
        </View>
      ...
      <View style={styles.statsGrid}>
        <StatItem icon="checkmark-done-circle" iconColor={Colors.success} value={progress.totalAnswered} label="総回答数" unit="問" />
        <StatItem icon="bulb" iconColor={Colors.warning} value={accuracy} label="正答率" unit="%" />
        <StatItem icon="flag" iconColor={Colors.primary} value={progress.completedQuestIds.length} label="クリア済み" unit="クエスト" />
        <StatItem icon="flame" iconColor={Colors.secondary} value={progress.streakDays} label="連続学習" unit="日" />
      </View>
      <Text style={styles.sectionTitle}>回答内訳</Text>
      <Card style={styles.breakdownCard}>
        <BreakdownRow label="正解" value={progress.correctAnswered} total={progress.totalAnswered} color={Colors.success} />
        <View style={styles.divider} />
        <BreakdownRow label="不正解" value={progress.totalAnswered - progress.correctAnswered} total={progress.totalAnswered} color={Colors.danger} />
      </Card>
      <Card style={styles.messageCard}>
        <Ionicons name="rocket" size={32} color={Colors.primary} />
        <Text style={styles.messageText}>
          {progress.totalAnswered === 0 ? 'さあ、最初のクエストに挑戦しよう！' : accuracy >= 80 ? '素晴らしい！この調子で志望校合格を目指そう！' : accuracy >= 60 ? '順調です！苦手分野を集中して攻略しよう！' : 'まだまだこれから！基礎クエストから積み上げよう！'}
        </Text>
      </Card>
    </ScrollView>
  );
...
function StatItem({ icon, iconColor, value, label, unit }: { icon: React.ComponentProps<typeof Ionicons>['name']; iconColor: string; value: number; label: string; unit: string }) {
  return (<Card style={styles.statItem}><Ionicons name={icon} size={28} color={iconColor} /><Text style={styles.statValue}>{value}</Text><Text style={styles.statUnit}>{unit}</Text><Text style={styles.statLabel}>{label}</Text></Card>);
}
...
function BreakdownRow({ label, value, total, color }: { label: string; value: number; total: number; color: string }) {
  return (
    <View style={styles.breakdownRow}>
      ...
      <View style={styles.breakdownBarWrapper}><ProgressBar progress={total > 0 ? value / total : 0} color={color} height={6} /></View>
...
const styles = StyleSheet.create({
  ...
  levelCircle: { width: 64, height: 64, borderRadius: Theme.borderRadius.full, backgroundColor: Colors.primary, alignItems: 'center', justifyContent: 'center' },
  ...
  statItem: { flex: 1, minWidth: '45%', alignItems: 'center', gap: 4, paddingVertical: Theme.spacing.lg },
  ...
  messageCard: { alignItems: 'center', gap: Theme.spacing.sm, padding: Theme.spacing.lg, backgroundColor: Colors.primaryLight, borderColor: Colors.primaryLight },
  messageText: { fontSize: Theme.fontSize.md, color: Colors.primary, fontWeight: Theme.fontWeight.medium, textAlign: 'center', lineHeight: 24 },
});
