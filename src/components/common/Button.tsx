...
export function Button({ label, onPress, variant = 'primary', size = 'md', disabled = false, loading = false, style, textStyle }: ButtonProps) {
  return (
    <TouchableOpacity
    ...
    >
      {loading
        ...
        : <Text style={[styles.label, styles[`labelSize_${size}`], styles[`labelVariant_${variant}`], disabled && styles.labelDisabled, textStyle]}>{label}</Text>
      }
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  base: { alignItems: 'center', justifyContent: 'center', borderRadius: Theme.borderRadius.lg },
  ...
  variant_outline: { backgroundColor: 'transparent', borderWidth: 2, borderColor: Theme.colors.primary },
